...
class NewArrivalItemsHome extends GetView<HomeControllerImp> {
  const NewArrivalItemsHome({Key? key}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      ...
      child: ListView.builder(
          itemCount: controller.newArrivalItems.length,
          scrollDirection: Axis.horizontal,
          itemBuilder: (context, i) {
            return ItemsHomeNewArrival(
                itemsModel: ItemsModel.fromJson(controller.newArrivalItems[i]));
          }),
    );
  }
}

class ItemsHomeNewArrival extends StatelessWidget {
  final ItemsModel itemsModel;
  const ItemsHomeNewArrival({Key? key, required this.itemsModel})
      : super(key: key);
